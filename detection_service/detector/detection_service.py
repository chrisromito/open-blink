...
class DetectionRepo:
    ...
    def result_to_out_message(
            self, device_id: str, detection_result: DetectionResult
    ) -> OutMessage:
        ...
        return OutMessage(topic=f"detection/{device_id}", data=json.dumps(meta))
...
class DetectionService:
    ...
    @classmethod
    def receive_in_messages(
            cls, msgs: list[InMessage], processor: Processor, repo: DetectionRepo
    ) -> list[OutMessage]:
        messages = [m for m in msgs if m.topic.startswith("image/")]
        if len(messages) < 1:
            logger.debug(
                "receive_in_messages: no image messages found, returning empty list"
            )
            return []
        # Group Messages by device ID
        logger.debug(f"DetectionService -> receive_in_messages: {messages}")
        receiver_timer = block_timer(
        ...
        for message in messages:
            ...
            device_message_map[device_id].append(
                ImageMessage(
                    device_id=device_id, payload=message.payload, timestamp=timestamp
                )
            )
        ...
        for device_id, image_messages in device_message_map.items():
            ...
            detection_results: list[DetectionResult] = instance.receive_image_messages(
                image_messages
            )
            group_messages: list[OutMessage] = [
                repo.result_to_out_message(device_id, dr) for dr in detection_results
            ]
    ...
    def receive_image_messages(
            self, msgs: list[ImageMessage]
        ...
        data_list = [json.loads(msg.payload.decode()) for msg in msgs]
        batch_results: BatchResult = self.processor.predict_batch(
            [x.get("file_name") for x in data_list]
        )
...
def get_files_in_directory(directory_path: str) -> list[str]:
    ...
    for entry in os.listdir(directory_path):
        ...
        if os.path.isfile(full_path) and (
                full_path.endswith(".jpg") or full_path.endswith(".jpeg")
        ):
