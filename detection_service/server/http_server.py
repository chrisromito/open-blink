...
async def run(
    processor: Processor, host: str = "0.0.0.0", port: int = settings.IMAGE_SOCKET_PORT
):
    ...
    async def _image_handler(
        ...
        try:
            while True:
                ...
                logger.debug("http_server -> image_handler() -> received image")
                ...
                logger.debug(
                    "http_server -> image_handler() -> running inference on image"
                )

                results: list[Detection] = processor.predict(
                    image
                )  # run_detection(image)
...
def send_test_image(
    image_bytes, host: str = "0.0.0.0", port: int = settings.IMAGE_SOCKET_PORT
):
